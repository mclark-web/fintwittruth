...
import { noonOpen, sessionClose } from "./quotes";
...
test("Monday noon is the noon print and Wednesday and Friday use the stored close", () => {
  ...
  assert.ok(spy);
  assert.equal(spy.wednesday, sessionClose("SPY", "2026-09-23"));
  assert.notEqual(spy.wednesday, noonOpen("SPY", "2026-09-23"));
  assert.equal(sessionClose("SPY", "2026-09-23"), 767.8099975585938);
});
