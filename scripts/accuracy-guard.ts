...
async function main() {
  ...
  for (const quote of live.quotes) {
    ...
    if (spec.wednesday?.session === "open") {
      const wednesday = spec.wednesday.date;
      const bars = await fetchChart(quote.symbol, unix(wednesday, 13), unix(wednesday, 22), "5m");
      const noon = readNoonBar(bars, wednesday);
      if (quote.wednesday == null || !pricesAgree(quote.wednesday, noon.open) || formatPrice(quote.wednesday) !== formatPrice(noon.open)) {
        mismatches.push(
          `${quote.symbol} Wednesday noon displayed ${quote.wednesday == null ? "blank" : formatPrice(quote.wednesday)} vs Yahoo ${formatPrice(noon.open)}`,
        );
      } else {
        console.log(`ok ${quote.symbol} Wednesday noon ${formatPrice(quote.wednesday)}`);
      }
    } else if (quote.wednesday == null) {
